...
<style lang="scss">
  ...
  .quantity > .cell, .buyingPrice > .cell, .remark > .cell, .totalPrice > .cell, .promoteDate > .cell {
    padding: 0 !important;
  ...
  .quantity > .cell input, .buyingPrice > .cell input, .remark > .cell input, .totalPrice > .cell input, .promoteDate > .cell input {
  ...
  .el-table__footer-wrapper .buyingPrice, .el-table__footer-wrapper .remark, .el-table__footer-wrapper .totalPrice, .el-table__footer-wrapper .quantity, .el-table__footer-wrapper .promoteDate {
    padding-left: 10px;
  }
  .status{
  ...
  export default {
    ...
    created: function () {
      if (this.$route.query.tid) {
        //查询单条订单信息
        this.$http(this.$_.merge({}, Action.Stock.OuterStockManage.singleInoutOrder, {
    ...
    methods: {
      ...
      FG_handleOrderSave(){
        ...
        if(this.FG_order.orderStatus==false){
          ...
          this.$http(this.$_.merge({}, Action.Stock.OuterStockManage.edit, {
            body:params
          })).then(response => {
            this.FG_order.orderStatusShow=true
            this.FG_order.orderStatus=false
      ...
      FG_handleOrderAudit(){
        this.$confirm('确定审核？')
          .then(_ => {
            this.$http(this.$_.merge({}, Action.Stock.OuterStockManage.audit, {
              //获取待更新数据
              body:{id:this.FG_order.data.tid,type :2}
            })).then(response => {
              this.$message({
                type: 'success',
                message: '审核成功'
