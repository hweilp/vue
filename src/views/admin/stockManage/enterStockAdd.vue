      ...
      <div class="content">
        ...
        <el-row :span="24" class="fg-content-detail">
          <el-row :span="24" class="fg-content-opr">
            <!-- <el-button :plain="true" type="primary" size="" @click="FG_handleOrderRefesh"><i
          ...
          <el-row :span="24">
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template scope="scope">
                  <!-- <el-input v-model="scope.row.promoteDateStr"  :readonly="FG_order.readonly" type="date"></el-input>-->
                  <el-date-picker
                    v-model="scope.row.promoteDate"
                    align="right"
                    format
                    :editable="false"
                    :readonly="FG_order.readonly"
                    v-if="scope.row.delete&&!FG_order.readonly"
                  >
                  </el-date-picker>
                </template>
              </el-table-column>
      ...
      FG_handleOrderSave(){
        ...
        if(merchantInoutOrderDetailDto.length==0){
          ...
          for (let i = 0; i < merchantInoutOrderDetailDto.length; i++) {
            ...
            if(merchantInoutOrderDetailDto[i].promoteDate==''){
              this.$message({
                type: 'warning',
                message: merchantInoutOrderDetailDto[i].name+' 生产日期不能为空'
              })
              return false
            }
          }
        ...
        this.$http(this.$_.merge({}, Action.Stock.EnterStockManage.add, {
          body:params
        })).then(response => {
          ...
          this.$confirm('保存成功，是否审核？')
            .then(_ => {
              this.$http(this.$_.merge({}, Action.Stock.EnterStockManage.audit, {
                ...
                this.$router.push({path:'/stock/enterStockManage'})
