        ...
        <div class="content">
          ...
          <el-row :span="24" class="fg-content-detail">
            <el-row :span="24" class="fg-content-opr">
              <router-link v-if="power.indexOf(16111101) > -1 ? true : false"  to="/stock/enterStockAdd" style="margin-right: 10px">
                <el-button :plain="true" type="primary" size=""><i
                  class="fa fa-plus"></i> 新增
                </el-button>
              </router-link>
              <!--  <router-link :to="{path:'/product/add',params:{pno:}}" style="margin-right: 10px">-->
              <el-button v-if="power.indexOf(16111102) > -1 ? true : false"  :plain="true" type="primary" size="" @click="FG_TABLE_handleAudit"><i
  ...
  export default {
    ...
    methods: {
      ...
      FG_TABLE_refreshData: function (option) {
        ...
        this.$http(this.$_.merge({}, Action.Stock.EnterStockManage.list, {
          params
        })).then(response => {
          //表格数据
      ...
      FG_TABLE_showEditRowForm: function () {
        ...
        if (this.FG_TABLE.multiSelectData.length === 1) {
          let tid = this.FG_TABLE.multiSelectData[0].id

          this.$router.push({path:'/stock/enterStockEdit',query:{tid:tid}})
      ...
      //高级查询获取日期
      getDate(val){
        if(val.dateEnd!='Invalid Date'&&val.dateStart!='Invalid Date'){
      ...
      //编辑
      handleEdit(val){
        this.$router.push({path:'/stock/enterStockEdit',query:{tid :val}})
      },
      //审核
            ...
            this.$confirm('确定审核？')
              .then(_ => {
                this.$http(this.$_.merge({}, Action.Stock.EnterStockManage.audit, {
                  //获取待更新数据
                  body:{id:this.FG_TABLE.multiSelectData[0].id,type :1}
                })).then(response => {
