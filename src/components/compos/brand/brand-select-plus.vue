...
<script type="text/javascript">
  ...
  export default {
    ...
    created: function () {
      ...
      //获取所有品牌列表数据
      if(this.$store.state.User.brand == null){
        this.init_brand()
      }else {
        this.brandOption =  this.brandList = this.$store.state.User.brand
      }
    },
    props: ['brandSelected'],
    watch: {
    ...
    methods: {
      ...
      init_brand(){
        this.$http(this.$_.merge({}, Action.Product.Info.brandList))
          .then(response => {
            ...
            this['User/INIT_USER_BRAND'](response.body)
            this.$store.dispatch('User/userBrand', response.body);
      ...
      handleSelectedBrandChange(val){
        this.$emit('update:brandSelected', Number(val))
      },
